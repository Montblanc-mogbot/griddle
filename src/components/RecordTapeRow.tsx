import type { FieldDef, RecordEntity } from '../domain/types';
...
export function RecordTapeRow(props: {
  record: RecordEntity;
  measures: string[];
  measureFieldByKey?: Record<string, FieldDef | undefined>;
  flags: TapeFlag[];
  onToggleFlag: (recordId: string, flagKey: string, value: boolean) => void;
  onUpdateMeasure: (measureKey: string, value: number | '') => void;
}) {
  const { record, measures, measureFieldByKey, flags, onToggleFlag, onUpdateMeasure } = props;
